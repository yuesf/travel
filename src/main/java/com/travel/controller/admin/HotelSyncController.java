...
import com.travel.service.HotelSyncService;
...
@Slf4j
@RestController
@RequestMapping("/api/v1/admin/hotels/sync")
@Tag(name = "酒店同步管理")
public class HotelSyncController {
    ...
    @Operation(summary = "手动同步酒店数据")
    public Result<SyncLog> manualSync(@PathVariable Long configId) {
        SyncLog syncLog = hotelSyncService.manualSync(configId);
